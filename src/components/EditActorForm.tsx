...
type EditActorFormProps = {
  actorId: string;
};

export default function EditActorForm({ actorId }: EditActorFormProps) {
  ...
  const updateActor = useActorsStore((state) => state.updateActor);

  const actor = useMemo(
    () => actors.find((currentActor) => currentActor.id === actorId),
    [actors, actorId]
  );
  ...
  const [loadingActor, setLoadingActor] = useState(!hasFetched);
  const [error, setError] = useState("");

  const today = new Date().toISOString().split("T")[0];

  useEffect(() => {
    const loadActorsIfNeeded = async () => {
      if (!hasFetched) {
        try {
  ...
  useEffect(() => {
    if (actor) {
      ...
      setBirthday(actor.birthDate.split("T")[0]);
  ...
  const handleSubmit = async (event: SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const success = await updateActor(actorId, {
        name: trimmedName,
        photo: trimmedPhoto,
      ...
      if (!success) {
        setError("Error updating the actor");
        setLoading(false);
        return;
      }
  ...
  if (!loadingActor && !actor) {
    return (
      <main className="min-h-screen bg-gray-100 px-6 py-10">
        <div className="mx-auto max-w-3xl rounded-2xl border border-red-200 bg-red-50 p-8 shadow-md">
          <p className="font-medium text-red-600">Actor not found.</p>
        </div>
      </main>
    );
  }
